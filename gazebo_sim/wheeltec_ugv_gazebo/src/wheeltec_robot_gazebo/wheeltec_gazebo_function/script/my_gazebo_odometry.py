...
class OdometryNode:
    ...
    def sub_robot_pose_update(self, msg):
        robot_prefix = rospy.get_namespace().strip("/")
        try:
            arrayIndex = msg.name.index(f'{robot_prefix}::base_footprint')         
    ...
    def timer_callback(self, event):
        ...
        #赋值
        cmd = Odometry()
        robot_prefix = rospy.get_namespace().strip("/")  # 获取机器人前缀
        cmd.header.stamp = self.last_recieved_stamp
        cmd.header.frame_id = 'world'
        cmd.child_frame_id = f'{robot_prefix}/base_footprint'
